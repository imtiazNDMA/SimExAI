...
class LLMEngine:
    ...
    def get_response(
        self, wing_id: str, phase_id: str, user_message: str,
        history: list[dict] | None = None,
        inject_ledger: str | None = None,
    ) -> str:
        # Builds a turn-mode prompt (orientation / phase briefing / discussion)
        # with scenario-first grounding, bounded history, targeted injects, and RAG.
        ...
```

The mandate is a compact role boundary, not a question checklist. The moderator
asks at most one command-level question per turn. Drafts that introduce unsupported
technical mechanics or stock phrasing receive one focused rewrite pass before return.
...
| `LMSTUDIO_REWRITE_TIMEOUT_SECONDS` | `60` | No-retry quality rewrite timeout |
| `LMSTUDIO_REWRITE_MAX_TOKENS` | `2500` | Quality rewrite budget |
